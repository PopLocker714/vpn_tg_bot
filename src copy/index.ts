...
app.post("*", async (c) => {
  ...
  if (context.preCheckoutQueryData) {
    await preCheckoutQueryController(context).catch((e) => {
      console.error("Error in preCheckoutQueryController:", e);
    });
  }

  if (context.successfulPayment) {
    await successfulPaymentController(context).catch((e) => {
      console.error("Error in successfulPaymentController:", e);
    });
  }
